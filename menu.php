...
<?php
                $id=12;
		$sql = "SELECT * from pavithra_catg where id = :id ";
...
?>
	...
	<?php include('modules/view/leftbar.php');?>
...
@keyframes Gradient {
   ...
   <table  class="display table table-striped table-bordered table-hover" style="width:100%;">
					<?php
					$sql1 =("SELECT * FROM pavithra_catg where status=1 ORDER BY id asc ");
                                        $query1= $objCon -> prepare($sql1);
                                        $query1-> execute();
